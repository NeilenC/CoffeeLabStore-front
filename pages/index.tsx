...
export default function Home() {
  ...
  const coffeeProducts = products.filter(product => product.category.name === 'Café');
  const kitProducts = products.filter(product => product.category.name === 'Accesorios');
  const otherProducts = products.filter(product => product.category.name !== 'Accesorios' && product.category.name !== 'Café' 
  ...
  return (
    <>
      ...
      <Cafeteras />
        <Typography variant='h5' sx={{p:5,textAlign: "center"}}> Nuestros cafés </Typography>
        <ProductsCard products={coffeeProducts}/>
        
       <Typography variant='h5' sx={{p:5, textAlign:"center" }}> Barista Tools</Typography>
       <ProductsCard products={kitProducts}/>

       <Typography variant='h5' sx={{p:5, textAlign: "center" }}> Otros que te pueden interesar </Typography>
